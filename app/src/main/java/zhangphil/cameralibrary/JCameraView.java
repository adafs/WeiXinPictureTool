...
import zhangphil.cameralibrary.listener.CaptureListener;
import zhangphil.cameralibrary.listener.ClickListener;
import zhangphil.cameralibrary.listener.ErrorListener;
import zhangphil.cameralibrary.listener.JCameraListener;
import zhangphil.cameralibrary.listener.TypeListener;
import zhangphil.cameralibrary.state.CameraMachine;
import zhangphil.cameralibrary.util.FileUtil;
import zhangphil.cameralibrary.util.LogUtil;
import zhangphil.cameralibrary.util.ScreenUtils;
import zhangphil.cameralibrary.view.CameraView;

import java.io.IOException;

import zhangphil.app.R;
...
//    private static final String TAG = "JCameraView";
    ...
    public JCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        iconSrc = a.getResourceId(R.styleable.JCameraView_iconSrc, R.drawable.ti_ic_camera);
    ...
    private void initView() {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.ti_camera_view, this);
        ...
        mCaptureLayout.setCaptureLisenter(new CaptureListener() {
            ...
            public void takePictures() {
                // 拍照
                if (!TClickUtil.isNotFastClick())  return;
...
//        });
    ...
    private void updateFlashLampState() {
        // 获取切换后的摄像头是否是前置
        boolean front = CameraInterface.getInstance().isFront();
        mFlashLamp.setVisibility(front ? INVISIBLE : VISIBLE);
        if (!front) {
            // 切换回后置摄像头时，更新闪光灯的状态
            setFlashRes();
        }
    ...
    private void setFlashRes() {
        switch (type_flash) {
            case TYPE_FLASH_AUTO:
                mFlashLamp.setImageResource(R.drawable.ti_ic_flash_auto);
                machine.flash(Camera.Parameters.FLASH_MODE_AUTO);
                break;
            case TYPE_FLASH_ON:
                mFlashLamp.setImageResource(R.drawable.ti_ic_flash_on);
                machine.flash(Camera.Parameters.FLASH_MODE_ON);
                break;
            case TYPE_FLASH_OFF:
                mFlashLamp.setImageResource(R.drawable.ti_ic_flash_off);
